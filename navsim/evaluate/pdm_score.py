...
def transform_trajectory(pred_trajectory: Trajectory, initial_ego_state: EgoState) -> InterpolatedTrajectory:
    ...
    relative_poses = np.array(pred_trajectory.poses, dtype=np.float64)  # values in ego frame
    relative_states = [StateSE2.deserialize(pose) for pose in relative_poses]
    absolute_states = relative_to_absolute_poses(initial_ego_state.rear_axle, relative_states)   # values in global frame
...
def pdm_score_from_interpolated_trajectory(
    ...
    # For DEBUG: try overriding heading values
    # pred_states[:, 2] = pdm_states[:, 2]
    # NOTE TODO depending on models
    if metric_cache.log_name.endswith("1_-2") or metric_cache.log_name.endswith("1_-1"):
        pred_states[:, 2] -= 0.0691
    trajectory_states = np.concatenate([pdm_states[None, ...], pred_states[None, ...]], axis=0)
    
    if metric_cache.log_name.endswith("1_-2") or metric_cache.log_name.endswith("1_-1"):
        initial_ego_state_tilt = copy.deepcopy(initial_ego_state)
        initial_ego_state_tilt.rear_axle.heading -= 0.0691
        initial_ego_state_tilt.center.heading -= 0.0691
        # print("correct ego heading")
    else:
        initial_ego_state_tilt = initial_ego_state
        # print("do not correct ego heading")
    # TODO uncomment
    simulated_states = simulator.simulate_proposals(trajectory_states, initial_ego_state_tilt)
    # simulated_states = trajectory_states
    ...
    pdm_result = scorer.score_proposals(
        ...
        # debug=False, # for distinguishing debugging
    )[pred_idx]
