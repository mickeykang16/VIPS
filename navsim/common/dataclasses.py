...
class Cameras:
    ...
    cam_r2: Camera
    cam_b0: Camera
    cam_infra0: Camera = field(default_factory=Camera)
    cam_infra1: Camera = field(default_factory=Camera)
...
class Lidar:
    ...
    def from_lidar_dict(
        ...
        lidar_sensor_name: str,  # noqa: ARG002 — kept for API consistency with dataloader
        sensor_names: List[str],
    ) -> Lidar:
        """
        Load lidar from V2XReal frame dict (binary .bin format).
        :param sensor_blobs_path: root directory to sensor data.
        :param lidar_dict: frame dict containing 'lidar_path' key.
        :param lidar_sensor_name: unused (for API consistency).
        :param sensor_names: list of sensor identifiers to load.
        :return: lidar point cloud dataclass (6 x N).
        """
        if "lidar_pc" not in sensor_names:
            return Lidar()
        lidar_path = lidar_dict.get("lidar_path")
        if not lidar_path:
            return Lidar()
        global_path = sensor_blobs_path / lidar_path
        if not global_path.exists():
            return Lidar()
        # KITTI-style binary: float32 [N, 4] (x, y, z, intensity)
        raw = np.fromfile(str(global_path), dtype=np.float32).reshape(-1, 4)
        n = raw.shape[0]
        lidar_pc = np.zeros((6, n), dtype=np.float32)
        lidar_pc[0] = raw[:, 0]  # x
        lidar_pc[1] = raw[:, 1]  # y
        lidar_pc[2] = raw[:, 2]  # z
        lidar_pc[3] = raw[:, 3]  # intensity
        # ring=0, lidar_id=0 (already zeros)
        return Lidar(lidar_pc=lidar_pc, lidar_path=lidar_path)
...
class EgoStatus:
    ...
    ego_yaw_rate: float = 0.0  # angular velocity (rad/s)
...
class AgentInput:
    ...
    # ── Optional metadata for agents that need global transforms ──────
    # Populated by _build_v2xreal_agent_input when sensor data is available.
    # All default to None so existing code is unaffected.
    ego2global_translation: Optional[npt.NDArray[np.float64]] = None   # (3,) [x, y, z]
    ego2global_rotation: Optional[npt.NDArray[np.float64]] = None      # (4,) quaternion wxyz
    lidar2ego_rotation: Optional[npt.NDArray[np.float64]] = None       # (4,) quaternion wxyz
    lidar2ego_translation: Optional[npt.NDArray[np.float64]] = None    # (3,) [x, y, z]
    timestamp: Optional[float] = None                                   # microseconds (original PKL unit)
    token: Optional[str] = None                                         # frame token
    ...
    def from_scene_dict_list(
        ...
        for frame_idx in range(num_history_frames):
            ...
            ego_status = EgoStatus(
                ...
                ego_acceleration=np.array(ego_dynamic_state[2:4], dtype=np.float32),
                driving_command=scene_dict_list[frame_idx]["driving_command"],
                ego_yaw_rate=float(ego_dynamic_state[4]) if len(ego_dynamic_state) > 4 else 0.0,
            )
...
class Annotations:
    ...
    is_v2xreal: bool = False  # Flag to indicate V2X-Real data (boxes already in global frame)

    def __post_init__(self):
        annotation_lengths: Dict[str, int] = {
            attribute_name: len(attribute) for attribute_name, attribute in vars(self).items() if attribute_name != 'is_v2xreal'
        }
        assert (
...
class Scene:
    ...
    def _build_annotations(cls, scene_frame: Dict) -> Annotations:
        ...
        return Annotations(
            ...
            is_v2xreal=scene_frame["anns"].get("is_v2xreal", False),  # Pass V2X-Real flag
        )
    ...
    def from_scene_dict_list(
        ...
        # NOTE: V2X-Real velocity is now computed upstream in
        # dataloader_v2xreal.compute_body_frame_velocity() before Scene creation.
        # No fallback velocity calculation needed here.
...
class SensorConfig:
    ...
    cam_infra0: Union[bool, List[int]] = False
    cam_infra1: Union[bool, List[int]] = False
...
def polyline_to_pdm_path(xy: np.ndarray) -> PDMPath:
    """
    xy: [N,2] global coordinates
    """
    if xy is None or len(xy) < 2:
        states = [StateSE2(0.0, 0.0, 0.0), StateSE2(1.0, 0.0, 0.0)]
    ...
    for i in range(len(xy)):
        ...
        if i < len(xy) - 1:
            nx, ny = float(xy[i + 1, 0]), float(xy[i + 1, 1])
            heading = math.atan2(ny - y, nx - x)
        else:
            px, py = float(xy[i - 1, 0]), float(xy[i - 1, 1])
            heading = math.atan2(y - py, x - px)
        states.append(StateSE2(x, y, heading))
    return PDMPath(states)


def polyline_to_polygon(xy: np.ndarray, width_m: float = 4.0) -> Polygon:
    """
    Simple drivable polygon for connector area: buffer centerline.
    """
    if xy is None or len(xy) < 2:
        return Polygon()

    ls = LineString(np.asarray(xy, dtype=np.float64))
    try:
        # cap_style=2: flat, join_style=2: mitre (shapely enums vary by version, so keep default if error)
        return ls.buffer(width_m / 2.0)
    except Exception:
        return Polygon()


@dataclass
class V2XLaneConnectorObject:
    """
    Minimal “MapObject-like” lane connector object.

    Keeps lane IDs for connectivity, and optionally can be "resolved" to actual lane objects
    by the map wrapper (recommended), so outgoing_edges/incoming_edges return real objects.
    """
    _id: str
    _centerline_xy: np.ndarray                    # [N,2] global
    _incoming_lane_ids: List[str] = field(default_factory=list)
    _outgoing_lane_ids: List[str] = field(default_factory=list)
    _polygon: Optional[Polygon] = None            # if None, derived from centerline buffer

    # resolved edge objects (set by wrapper)
    _incoming_edges: List["V2XMapObject"] = field(default_factory=list, repr=False)
    _outgoing_edges: List["V2XMapObject"] = field(default_factory=list, repr=False)
    ...
    def centerline_xy(self) -> np.ndarray:
        return self._centerline_xy

    @property
    def polygon(self) -> Polygon:
        # If polygon not provided, derive from centerline.
        if self._polygon is None:
            self._polygon = polyline_to_polygon(self._centerline_xy, width_m=4.0)
        return self._polygon

    @property
    def baseline_path(self) -> PDMPath:
        return polyline_to_pdm_path(self._centerline_xy)

    @property
    def incoming_lane_ids(self) -> List[str]:
        return self._incoming_lane_ids

    @property
    def outgoing_lane_ids(self) -> List[str]:
        return self._outgoing_lane_ids
    
    # --- connectivity as objects (resolved by wrapper) ---
    @property
    ...
    def resolve_edges(self, lane_id_to_obj: dict) -> None:
        """
        Called by wrapper after all lanes are built/cached.
        """
        self._incoming_edges = [lane_id_to_obj[lid] for lid in self._incoming_lane_ids if lid in lane_id_to_obj]
        self._outgoing_edges = [lane_id_to_obj[lid] for lid in self._outgoing_lane_ids if lid in lane_id_to_obj]
