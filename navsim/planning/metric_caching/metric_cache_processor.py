...
class MetricCacheProcessor:
    ...
    def _build_pdm_observation(
        ...
        # V2X-Real does not have traffic light data, so disable traffic light computation
        compute_traffic_light = interpolated_traffic_light_data is not None and len(interpolated_traffic_light_data) > 0
        pdm_observation.update_detections_tracks(
            interpolated_detection_tracks,
            interpolated_traffic_light_data,
            route_lane_dict,
            compute_traffic_light_data=compute_traffic_light,
        )
    ...
    def compute_and_save_metric_cache(self, scenario: NavSimScenario) -> Optional[CacheMetadataEntry]:
        ...
        try:
            ...
            logger.warning(
                f"Skipping scenario {scenario.token}: {e}"
            )
            return None
        metric_cache.dump()
    ...
    def compute_metric_cache(self, scenario: NavSimScenario) -> MetricCache:
        ...
        return MetricCache(
            ...
            driving_command = scenario.get_driving_command_at_iteration(0)
        )
