...
tracked_object_types: Dict[str, TrackedObjectType] = {
    ...
    # V2X-Real dataset object types mapping
    "car": TrackedObjectType.VEHICLE,
    "truck": TrackedObjectType.VEHICLE,
    "bus": TrackedObjectType.VEHICLE,
    "motorcycle": TrackedObjectType.VEHICLE,
    "bicycle_v2x": TrackedObjectType.BICYCLE,
}
...
def ego_status_to_ego_state(
    ...
    return EgoState.build_from_rear_axle(
        ...
        angular_vel=ego_status.ego_yaw_rate,
    )
...
def annotations_to_detection_tracks(annotations: Annotations, ego_state: EgoState) -> DetectionsTracks:
    ...
    # V2X-Real data is already in global frame, so skip transformation
    is_v2xreal = getattr(annotations, 'is_v2xreal', False)
    # DEBUG: print flag status (disabled for performance)
    # if is_v2xreal and len(annotations.boxes) > 0:
    #     print(f"[DEBUG] V2X-Real mode: {is_v2xreal}, num_boxes: {len(annotations.boxes)}, first_heading: {annotations.boxes[0][8]:.4f}")
    
    if is_v2xreal:
        # For V2X-Real: boxes are already [x_global, y_global, z, l, w, h, vx, vy, heading_global]
        # Note: heading is at index 8, not BoundingBoxIndex.HEADING (which is 6)
        track_boxes = []
        for gt_box in annotations.boxes:
            oriented_box = OrientedBox(
                StateSE2(gt_box[0], gt_box[1], gt_box[8]),  # x, y at 0,1; heading at 8
                gt_box[3],  # length
                gt_box[4],  # width
                gt_box[5],  # height
            )
            track_boxes.append(oriented_box)
    else:
        # For NuPlan: transform from ego-relative to global
    ...
    for track_idx, track_box in enumerate(track_boxes):
        ...
        if track_type in AGENT_TYPES:
            ...
            # V2X-Real velocity is already in global frame, don't rotate
            if not getattr(annotations, 'is_v2xreal', False):
...
def sample_future_indices(
    ...
    step_size = max(1, int(num_intervals // num_samples))
