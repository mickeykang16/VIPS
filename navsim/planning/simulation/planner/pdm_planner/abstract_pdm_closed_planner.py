...
from navsim.planning.simulation.planner.pdm_planner.vis_utils import debug_plot_lane_and_proposals, plot_proposal_paths, plot_proposal_refined_trajectories, plot_proposal_trajectories
class AbstractPDMClosedPlanner(AbstractPDMPlanner):
    ...
    def __init__(
        ...
        :param use_pdms_v1: If True, use PDMS v1 metrics instead of EPDMS v2
        """
        ...
        self._use_pdms_v1: bool = use_pdms_v1
        ...
        scorer_config = PDMScorerConfig(use_pdms_v1=use_pdms_v1)
        self._scorer = PDMScorer(proposal_sampling, config=scorer_config)
        ...
        if create_new_proposals:
            ...
            if False:
                plot_proposal_paths(
                    ...
                    centerline_discrete_path=self._centerline.discrete_path if self._centerline else None,
                    ego_state=ego_state,
                    out_path=f"exp_debug/proposal_paths/proposal_paths_iter{self._iteration}.png",
                    title=f"Proposal Paths (iter={self._iteration}, n={len(proposal_paths)})",
                )
    ...
    def _get_closed_loop_trajectory(
        ...
        # Save intermediate proposal data for ProposalCache
        self._last_proposals_array = proposals_array
        self._last_simulated_proposals = simulated_proposals_array
        self._last_pdm_results = pdm_results
        self._last_proposal_scores = proposal_scores
