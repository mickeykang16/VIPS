...
class PDMDrivableMap(PDMOccupancyMap):
    ...
    def from_simulation(cls, map_api: AbstractMap, ego_state: EgoState, map_radius: float = 50) -> PDMDrivableMap:
        ...
        lane_area = map_api.get_proximal_map_objects(position, map_radius, lane_layers)
        ...
        # 2b. Direct Lane Polygons (fallback when roadblocks are unavailable)
        if len(drivable_area[SemanticMapLayer.ROADBLOCK]) == 0 and len(drivable_area[SemanticMapLayer.ROADBLOCK_CONNECTOR]) == 0:
            for map_layer in lane_layers:
                polygons_, polygon_tokens_ = extract_map_layer(lane_area[map_layer])
                polygons.extend(polygons_)
                polygon_tokens.extend(polygon_tokens_)
