...
from navsim.common.dataclasses import V2XLaneConnectorObject


def _compute_cumulative_distances(states: List[StateSE2]) -> np.ndarray:
    coords = np.array([[s.x, s.y] for s in states])
    diffs = np.diff(coords, axis=0)
    seg_lengths = np.linalg.norm(diffs, axis=1)
    return np.concatenate([[0.0], np.cumsum(seg_lengths)])


def _unwrap_headings(headings: np.ndarray) -> np.ndarray:
    return np.unwrap(headings)


def interpolate_discrete_path_linear(
    discrete_path: List[StateSE2],
    target_spacing: float = 0.5,
) -> List[StateSE2]:
    """For straight lane segments: linear interpolation to reduce point spacing to target_spacing."""
    if len(discrete_path) < 2:
        return list(discrete_path)

    cum_dist = _compute_cumulative_distances(discrete_path)
    total_length = cum_dist[-1]
    if total_length < 1e-6:
        return list(discrete_path)

    coords = np.array([[s.x, s.y] for s in discrete_path])
    headings = _unwrap_headings(np.array([s.heading for s in discrete_path]))

    num_points = max(int(np.ceil(total_length / target_spacing)) + 1, 2)
    query_dist = np.linspace(0.0, total_length, num_points)

    x_interp = np.interp(query_dist, cum_dist, coords[:, 0])
    y_interp = np.interp(query_dist, cum_dist, coords[:, 1])
    h_interp = np.interp(query_dist, cum_dist, headings)

    result = []
    for x, y, h in zip(x_interp, y_interp, h_interp):
        h_norm = np.arctan2(np.sin(h), np.cos(h))
        result.append(StateSE2(float(x), float(y), float(h_norm)))
    return result


def interpolate_discrete_path_cubic(
    discrete_path: List[StateSE2],
    target_spacing: float = 0.5,
) -> List[StateSE2]:
    """For curved lane-connector segments: cubic spline interpolation for a smooth curve."""
    if len(discrete_path) < 2:
        return list(discrete_path)
    ...
    # Remove duplicate distances (avoids interp1d/CubicSpline errors)
    unique_mask = np.diff(cum_dist, prepend=-1.0) > 1e-6
    cum_dist_u = cum_dist[unique_mask]
    coords_u = coords[unique_mask]
    headings_u = headings[unique_mask]

    if len(cum_dist_u) < 3:
        return interpolate_discrete_path_linear(discrete_path, target_spacing)

    cs_x = CubicSpline(cum_dist_u, coords_u[:, 0], bc_type='natural')
    cs_y = CubicSpline(cum_dist_u, coords_u[:, 1], bc_type='natural')
    cs_h = CubicSpline(cum_dist_u, headings_u, bc_type='natural')

    num_points = max(int(np.ceil(total_length / target_spacing)) + 1, 2)
    query_dist = np.linspace(0.0, total_length, num_points)

    x_interp = cs_x(query_dist)
    y_interp = cs_y(query_dist)
    h_interp = cs_h(query_dist)
...
def is_connector_lane(lane) -> bool:
    """
    Determine whether a lane is a connector (curved segment).
    1) V2X-Real: a V2XLaneConnectorObject instance is a connector
    2) nuPlan: a class whose name contains 'Connector' is a connector
    3) fallback: treat as connector if heading variation > 15 deg
    """
    # V2X-Real connector
    if isinstance(lane, V2XLaneConnectorObject):
        return True

    # nuPlan connector (e.g. NuPlanLaneConnector)
    if 'Connector' in type(lane).__name__:
        return True

    # fallback: heading range heuristic
    raw_path = lane.baseline_path.discrete_path
    if len(raw_path) >= 3:
        headings = np.array([s.heading for s in raw_path])
        heading_range = np.ptp(np.unwrap(headings))
        if heading_range > np.deg2rad(15):
            return True
    return False


def interpolate_lane_path(
    lane,
    target_spacing: float = 0.5,
) -> List[StateSE2]:
    """
    Apply the appropriate interpolation based on lane type.
    - connector/curved -> cubic spline
    - normal lane/straight -> linear
    """
    raw_path = lane.baseline_path.discrete_path
    if is_connector_lane(lane):
        return interpolate_discrete_path_cubic(raw_path, target_spacing)
    else:
        return interpolate_discrete_path_linear(raw_path, target_spacing)
...
def get_current_roadblock_candidates(
    ...
    if not roadblock_candidates:
        for layer in layers:
            ...
            # Skip if roadblock_id_ is None (e.g., V2X-Real has no roadblocks)
            if roadblock_id_ is None:
                continue
