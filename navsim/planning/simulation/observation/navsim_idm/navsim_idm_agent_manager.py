    ...
    def propagate_agents(
        ...
        for agent_token, agent in self.agents.items():
            if agent.is_active(iteration) and agent.has_valid_path():
                ...
                # Some V2X-Real lane geometries put the snapped agent slightly off
                # the centerline buffer; rather than crash the whole eval, skip
                # propagation for this agent and let it stay in place this iteration.
                if not intersecting_agents.contains(agent_token):
                    continue
