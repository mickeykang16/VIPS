...
def build_idm_agents_on_map_rails(
    ...
    for agent in detections.tracked_objects.get_tracked_objects_of_type(TrackedObjectType.VEHICLE):
        ...
        if agent.track_token not in unique_agents:
            ...
            # Some V2X-Real lanes may produce degenerate (≤1-point) baseline paths.
            # nuPlan's IDMAgent builds an InterpolatedPath which asserts len > 1,
            # so try to build the agent first and only register it on success.
            try:
                idm_agent = IDMAgent(
                    start_iteration=0,
                ...
                logger.debug(f"Skipping IDM agent {agent.track_token} (degenerate route): {e}")
                continue

            # Register agent only after successful IDMAgent construction
            ...
            unique_agents[agent.track_token] = idm_agent
